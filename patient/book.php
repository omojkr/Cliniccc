...
$mysqli = new mysqli('localhost','root','','bookingcalendar');
    ...
    if($stmt->execute()){
        ...
        if($result->num_rows>0){
            ...
            $stmt->execute();
            $msg="<div class='alert alert-success'>Appointment is pending!</div>";
            $bookings[]=$timeslot;
            $stmt->close();
            $mysqli->close();
        }
...
<br>
    <div class="container">   
        <h1 class="text-center">Book An Appointment: <?php echo date('F d,Y',strtotime($date)); ?></h1>
        <center>       
        <div class="row">
            <div class="col-md-12">
...
if(isset($_POST['book.php'])){
    ...
    $select = "SELECT * FROM bookings WHERE timeslot = '$timeslot'";
    $result = mysqli_query($con,$select);

    if(mysqli_num_rows($result) > 0){
        echo '<script type = "text/javascript">';
        echo 'alert("Timeslot Already Taken")';
        echo 'window.location.href = "book.php"';
        echo '</script>';
    }else{
        $book = "INSERT INTO bookings (name, reason, timeslot, date, status) VALUES ('$name', '$reason', '$timeslot', '$date', 'pending')";
        mysqli_query($con, $book);
        echo '<script type = "text/javascript">';
        echo 'alert("Appointment is now pending")';
        echo 'window.location.href = "book.php"';
        echo '</script>';
    }
}
